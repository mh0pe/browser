...
pub fn asGraphics(self: *Polygon) *GraphicsElement {
    return self._proto._proto;
}
pub fn asSvg(self: *Polygon) *Svg {
    return self.asGraphics()._proto;
}
pub fn asElement(self: *Polygon) *Element {
    return self.asSvg()._proto;
}
pub fn asConstElement(self: *const Polygon) *const Element {
...
pub fn get_points(self: *Polygon) []const u8 {
    return self.asConstElement().getAttributeSafe(comptime String.wrap("points")) orelse "";
}

pub fn set_points(self: *Polygon, value: []const u8, frame: *Frame) !void {
    try self.asElement().setAttributeSafe(comptime String.wrap("points"), String.wrap(value), frame);
}
...
pub fn getBBox(self: *Polygon, frame: *Frame) !*DOMRect {
    const pts = self.asConstElement().getAttributeSafe(comptime String.wrap("points")) orelse
        return DOMRect.init(0, 0, 0, 0, frame);
    const bbox = parsePointsBBox(pts) orelse return DOMRect.init(0, 0, 0, 0, frame);
    return DOMRect.init(bbox[0], bbox[1], bbox[2], bbox[3], frame);
}
...
pub const JsApi = struct {
    ...
    pub const getBBox = bridge.function(Polygon.getBBox, .{});
    pub const points = bridge.accessor(Polygon.get_points, Polygon.set_points, .{});
};
