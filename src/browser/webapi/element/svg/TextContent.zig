...
fn resolveFontSize(element: *Element, frame: *Frame) f64 {
    const style = CSSStyleDeclaration.init(element, true, frame) catch return 16;
    const val = style.getPropertyValue("font-size", frame);
    if (val.len == 0) return 16;
    return parseCssFloat(val);
}

pub fn getNumberOfChars(self: *TextContent, frame: *Frame) u32 {
    const text = self.asNode().getTextContentAlloc(frame.call_arena) catch return 0;
    return @intCast(std.unicode.utf8CountCodepoints(text) catch 0);
}

pub fn getComputedTextLength(self: *TextContent, frame: *Frame) f64 {
    const text = self.asNode().getTextContentAlloc(frame.call_arena) catch return 0;
    const font_size = resolveFontSize(self.asElement(), frame);
    return text_shaper.measureWidth(text, font_size, "sans-serif");
}

pub fn getSubStringLength(self: *TextContent, offset: u32, count: u32, frame: *Frame) f64 {
    const text = self.asNode().getTextContentAlloc(frame.call_arena) catch return 0;
    const font_size = resolveFontSize(self.asElement(), frame);
