...
pub fn set_d(self: *Path, value: []const u8, frame: *Frame) !void {
    try self.asElement().setAttributeSafe(comptime String.wrap("d"), String.wrap(value), frame);
}

pub fn getBBox(self: *Path, frame: *Frame) !*DOMRect {
    const d = self.asConstElement().getAttributeSafe(comptime String.wrap("d")) orelse
        return DOMRect.init(0, 0, 0, 0, frame);
    const bbox = path_parser.computeBBox(d) orelse return DOMRect.init(0, 0, 0, 0, frame);
    return DOMRect.init(bbox.min_x, bbox.min_y, bbox.width(), bbox.height(), frame);
}
