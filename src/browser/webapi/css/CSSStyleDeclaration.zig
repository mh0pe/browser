...
fn setPropertyImpl(self: *CSSStyleDeclaration, property_name: []const u8, value: []const u8, important: bool, frame: *Frame) !void {
    ...
    if (std.mem.eql(u8, normalized, "font")) {
        try expandFontShorthand(self, value, important, frame);
        return;
    }
...
fn expandFontShorthand(self: *CSSStyleDeclaration, value: []const u8, important: bool, frame: *Frame) !void {
    ...
    if (font_style.len > 0) try self.setPropertyImplDirect("font-style", font_style, important, frame);
    if (font_weight.len > 0) try self.setPropertyImplDirect("font-weight", font_weight, important, frame);
    if (font_size.len > 0) try self.setPropertyImplDirect("font-size", font_size, important, frame);
    if (line_height.len > 0) try self.setPropertyImplDirect("line-height", line_height, important, frame);
    if (font_family.len > 0) try self.setPropertyImplDirect("font-family", font_family, important, frame);
}

// Like setPropertyImpl but skips shorthand expansion (used by shorthand expanders).
fn setPropertyImplDirect(self: *CSSStyleDeclaration, property_name: []const u8, value: []const u8, important: bool, frame: *Frame) !void {
