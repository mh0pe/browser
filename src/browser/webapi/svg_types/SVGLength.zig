...
pub fn setValue(self: *SVGLength, value: f64, frame: *Frame) !void {
    self.syncFromAttr();
...
pub fn getValueAsString(self: *SVGLength, frame: *Frame) ![]const u8 {
    self.syncFromAttr();
    const suffix = unitSuffix(self._unit_type);
    var buf: [64]u8 = undefined;
    const num = formatSvgFloat(&buf, self._value);
    return std.fmt.allocPrint(frame.call_arena, "{s}{s}", .{ num, suffix });
}

pub fn newValueSpecifiedUnits(self: *SVGLength, unit_type: u16, value: f64, frame: *Frame) !void {
    self._unit_type = unit_type;
    self._value = value;
    try self.writeBack(frame);
}

pub fn convertToSpecifiedUnits(self: *SVGLength, unit_type: u16, frame: *Frame) !void {
    self.syncFromAttr();
...
fn writeBack(self: *SVGLength, frame: *Frame) !void {
    if (self._element) |elem| {
        const suffix = unitSuffix(self._unit_type);
        var buf: [64]u8 = undefined;
        const num = formatSvgFloat(&buf, self._value);
        const str = try std.fmt.allocPrint(frame.call_arena, "{s}{s}", .{ num, suffix });
        try elem.setAttributeSafe(self._attr_name, String.wrap(str), frame);
    }
}
