...
pub fn init(x: ?f64, y: ?f64, z: ?f64, w: ?f64, frame: *Frame) !*DOMPoint {
    return frame._factory.create(DOMPoint{
        ._x = x orelse 0,
        ._y = y orelse 0,
        ._z = z orelse 0,
        ._w = w orelse 1,
...
pub fn fromPoint(x: ?f64, y: ?f64, z: ?f64, w: ?f64, frame: *Frame) !*DOMPoint {
    return init(x, y, z, w, frame);
}

pub fn matrixTransform(self: *const DOMPoint, matrix: *const DOMMatrix, frame: *Frame) !*DOMPoint {
    const m = matrix._proto._m;
    const x = self._x;
    const y = self._y;
    const z = self._z;
    const w = self._w;
